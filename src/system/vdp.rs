...
impl VDP {
    ...
    pub fn restore_state(&mut self, state: &serde_json::Value) {
        ...
        self.registers
            .from_base64(state["registers"].as_str().unwrap());
    ...
    fn get_sprite_size(&self) -> SpriteSize {
        let bsize = VDPRegisterModeControl2::from_bits_truncate(self.registers[1])
    ...
    pub fn render(&mut self) {
        ...
        for y in 0..28 {
            for x in 0..32 {
                ...
                let pattern = self.get_tile_pattern(bg.pattern, if bg.palette { 1 } else { 0 });

                // blit patterns to screen
                let px: isize = (hscroll + (x as isize) * 8).rem_euclid(256);
                let py: isize = ((y as isize) * 8 - vscroll).rem_euclid(224);

                self.blit_to_screen_ex(px, py, pattern, bg.horizontal_flip, bg.vertical_flip);
    ...
    fn execute_command_word(&mut self, addr: u16, code: u8) {
        ...
        match code {
            ...
            _ => {
                panic!("unhandled VDP command code: {}", code)
            }
        }
    }
