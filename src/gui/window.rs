...
use ggez;
use ggez::event::{Button, GamepadId, KeyCode, KeyMods};
...
impl event::EventHandler for EmulatorWindow {
    ...
    fn update(&mut self, ctx: &mut Context) -> GameResult {
        ...
        self.cpu_time.update(cpu_start.elapsed().as_micros() as f64);
    ...
    fn draw(&mut self, ctx: &mut Context) -> GameResult {
        ...
        graphics::draw(
            ...
            &graphics::Text::new((
                format!(
                    "FPS: {:1.1} CPU: ({:6.0}us {:6.0}us {:6.0}us)",
                    fps,
                    self.cpu_time.min(),
                    self.cpu_time.mean(),
                    self.cpu_time.max(),
        ...
        let rlcd = graphics::Mesh::new_rectangle(
            ctx,
        ...
        */
        ...
        for pi in 0..2 {
            for ci in 0..16 {
                ...
                let r = graphics::Rect::new(bx + (ci as f32) * w, by + (pi as f32) * h, w, h);
                let rlcd = graphics::Mesh::new_rectangle(
                    ctx,
        ...
        for py in 0..16 {
            for px in 0..16 {
                ...
                graphics::draw(
                    ...
                    &im,
                    graphics::DrawParam::new()
                        .dest(glam::vec2(bx + (px as f32) * 8.0, by + (py as f32) * 8.0)),
                )?;
            }
        }
